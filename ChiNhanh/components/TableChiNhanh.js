...
class TableChiNhanh extends Component {
  constructor(props) {
    super(props);
    this.state = {
      listColTextalign: ["POPULATION"],
    ...
    return {
      ...
      style: {
        background:
          rowInfo == undefined
            ? ""
            : that.state.selectedRows.has(rowInfo.original.BRANCH_CODE)
            ? "#dbe1ec"
            : "",
        color:
          rowInfo == undefined
  ...
  fetchData(state, instance) {
    ...
    if (this.state.firstRender) {
      ...
      RestfulUtils.posttrans("/banking/getListBranch", { ...data }).then(
        (resData) => {
      ...
      this.state.selectedRows.forEach((key, value, set) => {
        new Promise((resolve, reject) => {
          ...
          let datadelete = {
            ...
            p_branch_code: data[0].BRANCH_CODE,
            p_bankcode: data[0].BANKCODE,
            p_address: data[0].ADDRESS,
            p_director: data[0].DIRECTOR,
            p_name: data[0].NAME,
            p_population: data[0].POPULATION,
  ...
  setColTextalign = (colName) => {
    const col = this.state.listColTextalign.filter((item) => item == colName);
    let xhtml = "right";
    if (col.length == 0) {
      xhtml = "left";
    }
        ...
        <div className="col-md-12">
          <ReactTable
            columns={[
              ...
              {
                Header: (props) => (
                  <div className="wordwrap" id="lblBank_code">
              ...
              {
                Header: (props) => (
                  <div className="wordwrap" id="lblDirectorName">
                    {this.props.strings.DIRECTOR_NAME}
                  </div>
                ),
                id: "DIRECTOR_NAME",
                ...
                Cell: ({ value }) => (
                  <div
                    className={"col-" + this.setColTextalign("DIRECTOR_NAME")}
                    style={{ float: this.setColTextalign("DIRECTOR_NAME") }}
                    id={"lbl" + value}
                  >
                    {value}
                  </div>
              ...
              {
                Header: (props) => (
                  <div className="wordwrap" id="lblPopulation">
                ...
                Cell: ({ value }) => (
                  <div
                    className={"col-" + this.setColTextalign("POPULATION")}
                    style={{ float: this.setColTextalign("POPULATION") }}
                    id={"lbl" + value}
                  >
...

              {
                Header: (props) => (
                  <div className="wordwrap" id="lblStatusdes">
                    {
                      this.props.strings[
                        getExtensionByLang("STATUSDES", this.props.lang)
                      ]
                    }
                  </div>
                ),
...
const decorators = flow([connect(stateToProps), translate("ChiNhanh")]);

module.exports = decorators(TableChiNhanh);
